...
int epool_test()
{
	...
	{
		printf(
				"Could not bind socket %d to address 'INADDR_ANY' and port %u: %m",
				sd, port);
		close(sd);
		return 5;
	}
	...
	for (;;)
	{
		...
		for (i = 0; i < rval; i++)
		{
			...
			if (events & EPOLLOUT)
			{
				if (fd != sd)
				{
					rc = snprintf(buffer, sizeof(buffer),
							"Hello socket %d from server socket %d!\n", fd, sd);
			...
			if (events & EPOLLIN)
			{
				if (fd == sd)
				{
					while ((clientsd = accept(sd, (struct sockaddr *) &peeraddr,
							&salen)) < 0)
					{
					...
					if (inet_ntop(AF_INET, &peeraddr.sin_addr.s_addr, buffer,
							sizeof(buffer)) != NULL)
					{
						printf(
								"Accepted connection from %s:%u, assigned new sd %d\n",
